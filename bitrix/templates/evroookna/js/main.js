...
jQuery(document).ready(function($) {
	...
	$('form[name=SIMPLE_FORM_5] input[type=submit]').live('click', function() {
        var str = $('form[name=SIMPLE_FORM_5]').serialize();

        var error = 0;
		$('form[name=SIMPLE_FORM_5] input[type=text]').each(function(){
		...
		if(error == 0){
			...
			$.ajax({
				...
				data: str,
				success: function(data) {
					$('#slider-form .index-form').html(data);
setTimeout(function(){ajax_time('consult', '#slider-form .index-form')}, tine_out);
				},
				error:  function(xhr, str){}
			});
	...
	$('form[name=SIMPLE_FORM_6] input[type=submit]').live('click', function() {
        var str = $('form[name=SIMPLE_FORM_6]').serialize();

        var error = 0;
		$('form[name=SIMPLE_FORM_6] input[type=text]').each(function(){
			if($(this).val().length == 0){
				error = 1;
				$(this).css("box-shadow", " 0 0 10px rgba(255,0,0,0.5)");
			}else{
		...
		if(error == 0){
			...
			$.ajax({
				type: 'POST',
				url: '/ajax/stoimost.php',
				data: str,
				success: function(data) {
					$('#order-left').html(data);
				},
				error:  function(xhr, str){}
			});
	...
	$('form[name=SIMPLE_FORM_7] input[type=submit]').live('click', function() {
        var str = $('form[name=SIMPLE_FORM_7]').serialize();

        var error = 0;
		$('form[name=SIMPLE_FORM_7] input[type=text]').each(function(){
			if($(this).val().length == 0){
				error = 1;
				$(this).css("box-shadow", " 0 0 10px rgba(255,0,0,0.5)");
			}else{
		...
		if(error == 0){
			...
			$.ajax({
				...
				success: function(data) {
					$('#сallback').html(data);
setTimeout(function(){ajax_time('perez', '#сallback')}, tine_out);
				},
				error:  function(xhr, str){}
			});
	...
	$('form[name=SIMPLE_FORM_8] input[type=submit]').live('click', function() {
        var str = $('form[name=SIMPLE_FORM_8]').serialize();

        var error = 0;
		$('form[name=SIMPLE_FORM_8] input.namename').each(function(){
			if($(this).val().length == 0){
				error = 1;
				$(this).css("box-shadow", " 0 0 10px rgba(255,0,0,0.5)");
			}else{
		...
		if(error == 0){
			...
			$.ajax({
				...
				success: function(data) {
					$('#call').html(data);
					setTimeout(function(){ajax_time('zamer', '#call')}, tine_out);
				},
				error:  function(xhr, str){}
			});
    ...
    $('#go, .vizov_zam').click( function(event){ // ловим клик по ссылки с id="go"
   ...
   $("input[name=phone], input[name=number], input[name=form_text_39], input[name=form_text_35], input[name=form_text_37], input[name=form_text_47], input[name=form_text_32]").inputmask("+7(999)9999999", {  "showMaskOnHover": false });
	    $('.carousel').carousel({
        interval: 5000
    })
});
